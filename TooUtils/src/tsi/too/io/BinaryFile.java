...
/**
 * Provides operations for a binary file using the services of a file with random access. 
 * The definitions on this class are independent of the structure of a specific file.
 * Therefore, the services can be used by any class that wants to use a structure and implement the services of a file with random access.
 * 
 * <p><b>ATTENTION:</b> The file structure in relation to the amount and types of data in the record is defined by the user of the class.
 * However, the size of the records is required to be fixed.
 * This restriction is fundamental for the correct functioning of the methods of the <code>BinaryFile</code> class.</p> 
 * 
...
public abstract class BinaryFile <E> {
	 ...
	 * @throws FileNotFoundException if the file does not open (due to protected recording, user without write privileges ...).
	 * 
	 * @since 0.36
	 */
	public void openFile(String fileName, OpenMode mode) throws FileNotFoundException {
	...
	/**
	 * Position the file pointer at the position assigned by <code>offset</code> considering the
	 * record zero as the first record in the file.
	 * 
	 * @param offset number corresponding to a record in the file.
	 * @throws IOException if {@code offset} is less than {@code 0} or if {@code offset} is greater than {@link #countRecords()}
	 *  or if an I/O error occurs.
	 *  
	 * @since 0.36
	 */
	public void seekRecord(long offset) throws IOException {
		if(offset < 0 || offset > countRecords())
	...
	public void removeRecord(int position) throws IOException {
		...
		// if the position is not the last, overwrite the destination record with the last one in the file.
	...
	public enum OpenMode{
		...
		/**
		 *  Read and write mode - synchronously. All updates to file content is written to the disk synchronously.
		 */
		READ_WRITE_D("rwd"), 

		/**
		 *  Read and write mode - synchronously. All updates to file content or meta data is written to the disk synchronously.
		 */
		READ_WRITE_S("rws"); 
